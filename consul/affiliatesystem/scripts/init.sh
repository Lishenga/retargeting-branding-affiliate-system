...
# # retry users
SAMPLE_FILE=$(cat ./etc/consul.d/configs/consul.sample.json)
...
consul agent -retry-join="provider=aws tag_key=name tag_value=consul-server" -config-file=/etc/consul.d/consul.json -retry-interval=5s
